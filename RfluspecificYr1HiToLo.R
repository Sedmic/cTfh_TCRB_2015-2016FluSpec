...
sum(xshareduvw$`ICOS+CD38+`)
sum(xshareduvw$`ICOS-CD38-`)
sum(xshareduvw$`CXCR5-`)
sum(xsharedabc$`ICOS+CD38+`)
sum(xsharedabc$`ICOS-CD38-`)
sum(xsharedabc$`CXCR5-`)

TCRfluspecific <- read.csv("ICOS-CD38-Y1LoToHiY2CumulativeFreq.csv")
TCRfluspecific$Subject <- as.factor(TCRfluspecific$Subject)
ggplot(data=TCRfluspecific, aes(x=Day_Arbitrary, y=Frequency, group=interaction(Subject,Cell), color=Cell, shape=Subject, fill=Cell)) +
  geom_line(size=1) +
  geom_point(size=5) +
  ...
  scale_color_manual(values=c("#999999", "#339900", "darkorange")) +
  scale_shape_manual(values=c(21, 7, 22, 5, 24)) +
  scale_fill_manual(values=c("#999999", "#339900", "darkorange")) +
  theme_bw() +
  ...
  ggtitle("ICOS-CD38- Year 1 Clones in Year 2") +
