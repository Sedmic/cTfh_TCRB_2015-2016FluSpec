...
xyshareddec <- xyshared[xyshared$Frequency.x < xyshared$Frequency.y, ]

xysharedAA <- xyshareddec[[1]]
...
xyzshareddec <- xyzshared[xyzshared$Frequency.y < xyzshared$Frequency.z, ]
colnames(xyzshareddec)[2] <- "Y3D7"
colnames(xyzshareddec)[3] <- "Y2D7"
colnames(xyzshareddec)[4] <- "Y1D7"
xyzshareddec <- xyzshareddec[c(1, 4, 3, 2)]

TCRmerge <- melt(xyzshareddec, id=c("CDR3.AminoAcid"))
...
ggplot(data=TCRmerge, aes(x=Day, y=Frequency, group=CDR3.AminoAcid, color=CDR3.AminoAcid)) +
  ...
  ggtitle("Decreasing ICOS+CD38+ Clones of 999") +
  labs(x="Day", y="Clonotypic frequency (%)")

c("#FF99FF", "#FF0000", "#FF6600", "#FFFF00", "#00FF00", "#339900", "#00FFCC", "#0033FF", "#9966CC", "#000000")
write.csv(xyzshareddec, file="RsinglecloneMultYearsDecreaseOverTime101hi.csv")
